...
def on_message(ws, message):
    try:
        ...
        if code != 0:
            ...
            with open('./demos/speech_AI_food/foodrec.mp3', 'ab') as f:
                f.write(audio)
                #print("iconm")
...
def on_open(ws):
    def run(*args):
        ...
        #print("------>开始发送文本数据")
        ws.send(d)
        if os.path.exists('./demos/speech_AI_food/foodrec.mp3'):
            os.remove('./demos/speech_AI_food/foodrec.mp3')
...
def Xinghou_speaktts(context="这个是合成音频程序"):
    ...
    #播放音频 PLAY AUDIO
    proc = Popen("sudo mplayer ./demos/speech_AI_food/foodrec.mp3", shell=True)
    proc.wait()
    time.sleep(0.5)
