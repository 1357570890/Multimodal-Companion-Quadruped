...
class Ws_Param(object):
    ...
    # 生成url Generate URL Generate timestamp in RFC1123 format
    def create_url(self):
        # 生成RFC1123格式的时间戳 Generate timestamp in RFC1123 format
        now = datetime.now()
        date = format_date_time(mktime(now.timetuple()))

        # 拼接字符串 Splicing strings Encrypt hmac-sha256
...
def xinghou_Image(imagetext):
        global answer,text
        imagedata = open("./demos/speech_AI_food/myrec.jpeg",'rb').read()
